...
void main() {
  group('LedStripCapability', () {
    ...
    test(
      ...
      () async {
        ...
        final writes = transport.writes
            ...
            .toList();
        /* ONE WRITE, NOT FOUR. Three of the four colours are the ones
         * hydration already read, and every stored write reaches flash. */
    ...
    /* ─── WRITES THAT ARE NOT MADE ────────────────────────────────────────
     * Every stored register is persisted by the firmware, so a write is a flash
     * write; the live pair is dragged against by a colour picker. Both paths
     * therefore skip a write that would change nothing. */
    ...
    test('a save forgets what the strips were showing', () async {
      ...
      const colour = Color16(0x1100, 0x2200, 0x3300);
      await bengle.previewLedStrip(front: colour);
      /* The save makes the firmware re-apply the stored pair, so the strips no
       * longer show the preview and the same colour must be sent again. */
      await bengle.setLedStrip(
        LedStripState(
          frontStrip: ZoneLedState(
